...
def manage_worker(worker_socket, worker_num, temps_queue, Mat1, Mat2, result, count, total_times, lock, worker_log, master_log):
    while not temps_queue.empty(): 
        ...
        for k in range(SIZE):
            current_result += Mat1[row][k] * Mat2[k][col]
        with lock:
            if success:
                result[row][col] = temp_result
                count[worker_num] += 1
                total_times[worker_num] += run_time
                log_print(master_log, f'Worker {worker_num + 1} : {current_location} = {current_result}')
                log_print(worker_logger, f'연산 성공 : {current_location}, 소요된 시간 {run_time}초')
                print(f'작업 {current_location} 성공')
            else:
                temps_queue.put(temp)
                log_print(master_log, f'Worker {worker_num + 1} : {current_location} 연산 실패, 다른 워커로 재할당. . .')
...
def main():
    ...
    for i in range(WORKERS):
        log_print(worker_log[i], f'worker {i + 1}이 처리한 작업 수 {count[i]}')
        log_print(worker_log[i], f'worker {i + 1}이 수행한 시간 {total_times[i]} 초')
    server.close()
